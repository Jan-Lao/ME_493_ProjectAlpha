...
void Agent::react(double rr){
    ...
    //For parameter variation purposes only
    alpha = 0.99*alpha;
...
////-------------------------run()-----------------------------
void Bandit::run(){
    ofstream fout("/Users/Jan/Desktop/ProjectAlpha.txt", fstream::trunc);
    start();
    p1->Qvectorinit();
    int moves = 0;
    ...
    //Examine for report purposes
    fout << endl << TestBArm0.size() << endl;
    fout << TestBArm1.size() << endl;
    fout << TestBArm2.size() << endl;
    
    cout << "Test B has been concluded" << endl << endl;
...
void TestA(){
    ...
    for(int i=0; i< big_number; i++){
        ...
        TestArr = TestArr * TestAAgent.alpha + TestArr*(1-TestAAgent.alpha); //=action,value learner
...
int main() {
    ...
    //Here in order to change parameter
    agent.alpha = 0.1;
